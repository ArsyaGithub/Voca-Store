...
import DeleteCartItemButton from "./DeleteItem"
...
import { getCoupon, getMyCoupon } from "@/lib/api/Coupun"
...
export function CartSheet() {
    ...
    useEffect(() => {
        ...
        } else if (couponCode && selectedAddressUid) {
        }
    }, [selectedItems]);
    const total =
        cart?.items
    ...
    const handleCheckout = async () => {
        if (selectedItems.length === 0) {
            notifyError("Pilih item dulu bang 😭")
            return
        }
