...
import { GenericEditDialog, type GenericEditField } from "@/components/GenericEdit"
import { formatRupiah } from "@/lib/utils"
import { addCoupon, deleteCoupon, editCoupon } from "@/lib/api/Coupun"
          ...
          onSubmit={async (values) => {
            try {
              if (!values.expires_at) {
                return { success: false, message: "Tanggal kadaluarsa wajib diisi" }
              }
              const isActive = values.is_active === true || values.is_active === "true"
              await editCoupon(editItem.id, {
                code: values.code,
                type: values.type === "percentage" || values.type === "fixed" ? values.type : (values.type as any),
